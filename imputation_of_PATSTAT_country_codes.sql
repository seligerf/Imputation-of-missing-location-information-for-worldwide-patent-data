/***************************************************************************************************************************
  ...
  Algorithm to impute missing country codes of first patent filings by inventors from subsequent filings
  ...
  Description : PostgreSQL code for PATSTAT to impute missing country information (country code) for first filings based on the inventor criterion. 
  ...
  Output: table CTRY_INV. The field 'source' indicates where the information on
  ...
  For table CTRY_APP, we applied the "inventor criterion", i.e. when the information applicants' country is missing, the algorithm looks into direct equivalents 
  and other subsequent filings for the information. Otherwise, information can be retrieved from inventors' country.
...
-- Tables containing appln_id, person_id, ctry_code for inventors and applicants, respectively
---------------------------------------------------------------------------------------------------
...
DROP INDEX IF EXISTS country_codes_inv_idx, country_codes_app_idx;
...
-- all priority filings with country information in PATSTAT
...
WHERE t1.nb_priorities = 1 AND NULLIF(ctry_code, '') IS NOT NULL );
...
FROM PRIORITY_FILINGS_inv t1 
JOIN country_codes_app t2 ON t1.appln_id = t2.appln_id  WHERE NULLIF(t2.ctry_code, '') IS NOT NULL) ;
...
-- table containing the information on priority filings and country information (if available)
...
SELECT  t_.appln_id, ctry_code, 1, type
FROM TABLE_INV t_
...
DROP TABLE IF EXISTS CTRY_INV;
CREATE TABLE CTRY_INV (
appln_id INTEGER DEFAULT NULL,
...
source SMALLINT DEFAULT NULL,
type TEXT DEFAULT NULL
); COMMENT ON TABLE CTRY_INV IS 'Inventors from priority filings of a given (po, year)';
 
-- F. Job done, insert into final table 
INSERT INTO CTRY_INV
SELECT t1.appln_id, t2.patent_office, t2.appln_filing_date, t2.appln_filing_year, ctry_code, source, type
...
CREATE INDEX CTRY_INV_APPLN_ID ON CTRY_INV(appln_id);
CREATE INDEX CTRY_INV_YEAR ON CTRY_INV(priority_year);
 
 
 
 
 
ALTER TABLE CTRY_INV ADD COLUMN country VARCHAR; 
UPDATE CTRY_INV SET country = 
CASE
...
CREATE INDEX CTRY_INV_COUNTRY ON CTRY_INV(country);
...
DROP table if exists PRIORITY_FILINGS1;
DROP table if exists SUBSEQUENT_FILINGS;
DROP table if exists EQUIVALENTS2;
DROP table if exists EARLIEST_EQUIVALENT2;
DROP table if exists EARLIEST_EQUIVALENT2_;
DROP table if exists OTHER_SUBSEQUENT_FILINGS3;
